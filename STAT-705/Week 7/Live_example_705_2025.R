...
#Below are elaborations of an ANOVA and ANCOVA model trying to chase biological realism....

#m5 <- lm(Body.mass.in.kg~Age.class*Sex,data=df.desoto.mod)
m5 <- lm(Body.mass.in.kg~Age.class*Sex+Amount.of.cropland.in.hectares+Post.harvest.abundance.estimates,data=df.desoto.mod)
#m5 <- lm(Body.mass.in.kg~Age.class*Sex+Amount.of.cropland.in.hectares*Sex+Post.harvest.abundance.estimates*Sex,data=df.desoto.mod)
#m5 <- lm(Body.mass.in.kg~Age.class*Sex+Amount.of.cropland.in.hectares*Sex*Age.class+Post.harvest.abundance.estimates*Sex*Age.class,data=df.desoto.mod)
summary(m5)
anova(m5)
...
# using 865 recorded deer body weights from Desoto NWR
# that were not used previously for model fitting
E.y.hat <- predict(m5,newdata=df.desoto.test)
y <- df.desoto.test$Body.mass.in.kg
...
# 865 recorded deer body weights from Desoto NWR that 
# were not used previously for model fitting
li <- predict(m5,newdata=df.desoto.test,interval="prediction",level=0.95)[,2]
ui <- predict(m5,newdata=df.desoto.test,interval="prediction",level=0.95)[,3]
...
plot(df.desoto.mod$Amount.of.cropland.in.hectares, e.hat)

shapiro.test(residuals(m1))

# P-value is essentially 0; derived quantity (residuals) are not normally distributed.
